...
REM ═══ NGROK (OPCIONAL) ═══
REM Verificar si existe token.txt para habilitar ngrok
set USE_NGROK=0
if exist "%~dp0token.txt" (
    set USE_NGROK=1
    echo [%date% %time%] Token de ngrok encontrado, habilitando tunel publico... >> "%LOGFILE%"
) else (
    echo [%date% %time%] No se encontro token.txt, modo solo local >> "%LOGFILE%"
)
